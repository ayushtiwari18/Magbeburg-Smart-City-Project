...
// ── IHK datasets (unchanged from v1) ─────────────────────────────────────────
const IHK_SECTORS = [
...
const TABS = [
  ...
  {id:"handwerk",     label:"🔧 Crafts (Handwerk)", icon:Wrench},
  {id:"consumer",     label:"🫱 Consumer Advisory", icon:HeartHandshake},
];

// ── Shared UI ─────────────────────────────────────────────────────────────────
type TooltipPayload = {name:string;value:number;color:string};
const DarkTooltip = ({active,payload,label}:{active?:boolean;payload?:TooltipPayload[];label?:string}) => {
...
function StatCard({icon:Icon,value,label,sub,color=C.blue,delta}:{
  ...
  return(
    <div style={{background:C.panel,border:`1px solid ${C.border}`,borderRadius:12,padding:"16px 18px",position:"relative",overflow:"hidden",transition:"box-shadow 0.2s"}}
