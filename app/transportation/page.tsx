...
const OPERATORS = [
  {
    ...
    // SVG icon as DivIcon HTML — tram symbol
...
function DonutChart({ segments }: { segments: { label: string; value: number; color: string }[] }) {
  ...
  if (total === 0) return <div className="flex items-center justify-center h-32"><div className="h-7 w-7 rounded-full border-4 border-slate-200 border-t-blue-500 animate-spin" /></div>;
  let angle = -Math.PI / 2;
      ...
      <div className="flex flex-col gap-1.5">
        {segments.map(s => (
          <div key={s.label} className="flex items-center gap-2">
            ...
            <span className="text-xs text-slate-400 tabular-nums ml-auto pl-3">{s.value}</span>
          </div>
        ))}
      </div>
    </div>
...
function CombinedPeakChart({ visible }: { visible: Set<string> }) {
  ...
  const W = 800; const H = 130; const PAD = 28;
      ...
      {Array.from({ length: 24 }, (_, h) => {
        ...
        return (
          <g key={h}>
            ...
            {h % 6 === 0 && <text x={x+barW/2} y={H-6} textAnchor="middle" fill="#94a3b8" fontSize="9">{String(h).padStart(2,'0')}:00</text>}
            {isPeak && <text x={x+barW/2} y={y-5} textAnchor="middle" fill="#f59e0b" fontSize="9" fontWeight="700">▲ {combined[h]}</text>}
          </g>
        );
      })}
    </svg>
...
export default function Transportation() {
  ...
  // Load GTFS per operator and populate map layers
  ...
  useEffect(() => {
    // Stagger loads to avoid hammering network
  ...
  return (
    <div className="bg-[#f8fafc] min-h-screen">
        ...
        <Container>
          <div className="py-5">
              ...
              {OPERATORS.map(op => {
                ...
                return (
                  ...
                  >
                    ...
                    <Icon size={11} />
                    <span>{op.label}</span>
                    <span className="opacity-70">— {op.type}</span>
                    {!loading && opStops[op.key] && (
                      <span className="ml-1 opacity-80">{opStops[op.key].length.toLocaleString()} stops</span>
                    )}
                  </button>
                );
              })}
            ...
            <div className="relative rounded-2xl overflow-hidden border border-slate-200 shadow-md">
              ...
              <div className="absolute bottom-3 left-3 z-[999] bg-white/95 backdrop-blur rounded-xl shadow-lg p-3 flex flex-col gap-1.5 border border-slate-100">
                ...
                {OPERATORS.map(op => (
                  <div key={op.key} className={`flex items-center gap-2 transition-opacity ${visible.has(op.key) ? "opacity-100" : "opacity-30"}`}>
                    <span className="w-4 h-4 rounded-full border-2 border-white shadow-sm flex-shrink-0" style={{ backgroundColor: op.color }}
              ...
              <div ref={mapRef} style={{ height: "60vh", minHeight: 420 }} />
            </div>

            {/* Source note */}
            <p className="text-[11px] text-slate-400 mt-2">
              Stop data from <strong>GTFS feeds</strong> — NASA GmbH / Nahverkehrsservice Sachsen-Anhalt.
              Click any stop marker for details. Toggle operators using the pills above.
            </p>
          </div>
        </Container>
      </section>
      ...
      {/* ── Analytics strip ── */}
      <section className="bg-[#061B46] py-12">
        <Container>
          <div className="flex items-center gap-2 mb-8">
            <TrendingUp className="h-5 w-5 text-blue-300" />
            <h2 className="text-xl font-bold text-white">Network Analytics</h2>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            ...
            <div className="md:col-span-2 rounded-2xl bg-white/5 border border-white/10 p-5">
              ...
              <p className="text-xs text-blue-400 mb-4">Source: KISS-MD · befoerderte-personen-der-mvb</p>
              {kissLoading ? (
                <div className="space-y-2">{Array.from({length:6}).map((_,i)=><div key={i} className="h-3.5 bg-white/10 rounded animate-pulse" />)}</div>
                ...
                <div className="space-y-2">
                  {ridershipData.map(d => (
                    <div key={d.year} className="flex items-center gap-3">
                      ...
                      <span className="text-xs text-blue-200 tabular-nums w-14 text-right">{(d.passengers/1e6).toFixed(1)}M</span>
                    </div>
                  ))}
                </div>
              )}
            ...
            <div className="rounded-2xl bg-white/5 border border-white/10 p-5">
              ...
              <p className="text-xs text-blue-400 mb-4">Live GTFS feeds</p>
              <DonutChart segments={donutData} />
            </div>

            {/* Combined peak chart — full width */}
            <div className="md:col-span-3 rounded-2xl bg-white/5 border border-white/10 p-5">
                ...
                <div className="flex flex-wrap gap-3">
                  {OPERATORS.map(op => (
                    <div key={op.key} className={`flex items-center gap-1.5 transition-opacity ${visible.has(op.key) ? "opacity-100" : "opacity-30"}`}>
                      <span className="w-2.5 h-2.5 rounded-sm" style={{ backgroundColor: op.color }} />
              ...
              <p className="text-xs text-blue-400 mb-3">Toggle operators above to filter · estimated from GTFS trip frequencies</p>
              <CombinedPeakChart visible={visible} />
            </div>

            {/* Per-operator peak hour cards */}
            {OPERATORS.map(op => {
              ...
              return (
                <div key={op.key}
                  ...
                  <div className="flex items-center gap-2 mb-2">
                    <div className="w-7 h-7 rounded-lg flex items-center justify-center" style={{ backgroundColor: op.color }}>
                  ...
                  <div className="grid grid-cols-3 gap-2 mt-3">
                    <div className="text-center">
                      <div className="text-lg font-bold tabular-nums" style={{ color: op.color }}>
                        {opLoading[op.key] ? "…" : (opStops[op.key]?.length ?? 0).toLocaleString()}
                      </div>
                      <div className="text-[10px] text-blue-400">stops</div>
                    </div>
                    <div className="text-center">
                      <div className="text-lg font-bold tabular-nums" style={{ color: op.color }}>
                        {String(peakH).padStart(2,'0')}:00
                      </div>
                      <div className="text-[10px] text-blue-400">peak hour</div>
                    </div>
                    <div className="text-center">
                      <div className="text-lg font-bold tabular-nums" style={{ color: op.color }}>{avgTrips}</div>
                      <div className="text-[10px] text-blue-400">avg trips/h</div>
                    </div>
                  </div>
                  {/* Mini sparkline */}
                  <div className="mt-3">
                    <svg viewBox="0 0 240 40" className="w-full" style={{ height: 40 }}>
                      <defs>
                      ...
                      {(() => {
                        ...
                        const pts = op.peakHours.map((v,h) => `${(h/23)*220+10},${38-(v/mx)*32}`);
                        const area = `M${pts[0]} ${pts.slice(1).map(p=>`L${p}`).join(" ")} L230,38 L10,38 Z`;
                        return (
                          <>
